...
contract Cash_Cow {
    ...
    IERC20 public cash_Token;
    address erctoken = 0x26D09B98f9c07B73Fa851d4E312d89Ef8B7a6D0b; 
    ...
    /** bonus **/
    uint256 public COMPOUND_BONUS = 3; /** 2.5% **/
    ...
    constructor(
        ...
        address payable _partner1,
        address payable _partner2,
        ...
        require(
            !isContract(_owner) &&
                !isContract(_project) &&
                !isContract(_partner1) &&
                !isContract(_partner2) &&
                !isContract(_marketing) &&
                !isContract(_buyback)
        );
        cash_Token = IERC20(erctoken);
        owner = _owner;
        project = _project;
        partner1 = _partner1;
        partner2 = _partner2;
    ...
    function hatchEggs(bool isCompound) public {
        ...
        if (isCompound) {
            ...
            if (specialEventBonus) {
                eggsForCompound = eggsForCompound.add(
                    eggsForCompound.mul(COMPOUND_SPECIAL_BONUS).div(
                        PERCENTS_DIVIDER
                    )
                );
            }
    ...
    function sellEggs() public {
        ...
        cash_Token.transfer(msg.sender, eggsPayout);
    ...
    function buyEggs(address ref, uint256 amount) public payable {
        ...
        cash_Token.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                if (specialEventBonus) {
                    refRewards = refRewards.add(
                        amount.mul(REFERRAL_EVENT_BONUS).div(PERCENTS_DIVIDER)
                    );
                }
                cash_Token.transfer(upline, refRewards);
    ...
    function payFees(uint256 eggValue) internal returns (uint256) {
        ...
        cash_Token.transfer(project, projectFee);
        cash_Token.transfer(partner1, partnerFee);
        cash_Token.transfer(partner2, partnerFee);
        cash_Token.transfer(buyback, buybackFee);
    ...
    function payFeesSell(uint256 eggValue) internal returns (uint256) {
        ...
        cash_Token.transfer(project, prj);
        cash_Token.transfer(marketing, mkt);
        cash_Token.transfer(buyback, bbk);
    ...
    function getFees(uint256 eggValue)
        public
        view
        returns (uint256 _projectFee, uint256 _partnerFee)
    {
    ...
    function _buyTickets(address userAddress, uint256 amount) private {
        ...
        if (specialEventBonus) {
            MAX_LOTTERY_TICKET = MAX_LOTTERY_TICKET.add(
                EVENT_MAX_LOTTERY_TICKET
            );
        }
        ...
        currentPot = currentPot.add(
            amount.mul(LOTTERY_PERCENT).div(PERCENTS_DIVIDER)
        );
    ...
    function chooseWinner() public {
        ...
        if (participants != 0) {
            ...
            for (uint256 i = 0; i < participants; i++) {
                ...
                uint256 range1 = range0.add(
                    ticketOwners[lotteryRound][
                        participantAdresses[lotteryRound][i]
                    ].div(1e18)
                );
            ...
            for (uint256 i = 0; i < participants; i++) {
                if ((random >= init_range[i]) && (random <= end_range[i])) {
                    /** winner found **/
                    address winnerAddress = participantAdresses[lotteryRound][
                        i
                    ];
                    ...
                    user.miners = user.miners.add(
                        eggsReward.div(EGGS_TO_HIRE_1MINERS_COMPOUND)
                    );
                    ...
                    uint256 back = currentPot.mul(LOTTERY).div(
                        PERCENTS_DIVIDER
                    );
                    cash_Token.transfer(buyback, back);
    ...
    function getLotteryHistory(uint256 index)
        public
        view
        returns (
            uint256 round,
        ...
        )
    {
        ...
        totalLotteryParticipants = lotteryHistory[index]
            .totalLotteryParticipants;
    ...
    function getLotteryInfo()
        ...
        returns (
            ...
            uint256 maxLotteryParticipants,
            uint256 totalLotteryTickets,
            uint256 lotteryTicketPrice,
            uint256 maxLotteryTicket,
    ...
    function getBalance() public view returns (uint256) {
        return cash_Token.balanceOf(address(this));
    }
    ...
    function PROMO_EVENT_SPECIAL(
        bool value,
        uint256 addCompoundBonus,
        uint256 addReferralEvent,
        uint256 addMaxTicket
    ) external returns (bool) {
        require(msg.sender == owner, "Admin use only.");
        require(
            addCompoundBonus <= 900,
            "Additional compound bonus max value is 900(90%)."
        ); /** 90% max **/
        require(
            addReferralEvent <= 100,
            "Additional referral bonus max value is 100(10%)."
        ); /** 10% max **/
        require(
            addMaxTicket <= 100,
            "Additional ticket bonus max value is 100 tickets."
        ); /** 100 max **/
