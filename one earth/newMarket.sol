...
contract Marketplace is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function cancelOrder(uint256 _tokenId) external {
        ...
        require(
            seller == msg.sender || msg.sender == owner(),
    ...
    function updateOrder(uint256 _tokenId, uint256 _priceInWei) external {
        require(
            orderByTokenId[_tokenId].seller == msg.sender,
    ...
    function executeOrder(uint256 _tokenId) external payable nonReentrant {
        ...
        if (bidByTokenId[_tokenId].price > 0) {
            _refundBid(
                bidByTokenId[_tokenId].bidder,
                bidByTokenId[_tokenId].price,
                _tokenId
            );
        }
    ...
    function cancelBid(uint256 _tokenId) external nonReentrant {
        ...
        require(
            bidder == msg.sender,
            "Marketplace: Only bidder can cancel the bid"
        );
    ...
    function acceptBid(uint256 _tokenId) external nonReentrant {
        ...
        require(
            msg.sender == orderByTokenId[_tokenId].seller,
            "Marketplace: Only seller can accept bid"
        );
    ...
    function removeOrder(address seller, uint256 _tokenId) internal {
        uint256 lastTokenId = ordersBySeller[seller][
    ...
    function getBidDetails(uint256 tokenId)
        public
        view
        returns (
            address bidder,
    ...
    function changeDividendContract(address _dividendTracker)
        external
        onlyOwner
    {
